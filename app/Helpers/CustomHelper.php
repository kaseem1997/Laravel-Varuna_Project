...
class CustomHelper{
    ...
    public static function getThemeName(){
        ...
        if(!empty($theme_data) && count((array)$theme_data) > 0){
    ...
    public static function computeFees() {
        ...
        if (!empty($cart['coupon'])) {
            ...
            // Else, Coupon is valid
                ...
                if (count((array)$couponProducts) == 0) {
...
// echo 'here'; die;
    ...
    public static function getCMSInsight($id='',$page_id='', $type='blogs', $limit='', $params=[]){
        ...
        $news_query = Blog::where(['status'=>1, 'type'=>$type])->orderBy('sort_order','desc');

        if(!empty($page_id)){
            $news_query->whereRaw("find_in_set($page_id,pages_id)");
        }
    ...
    public static function get_words_count($html){
       $stripped = strip_tags($html);
       $decoded  = html_entity_decode($stripped);
       return str_word_count($decoded);
   }



   public static function carrerListApi($job_id=''){

        $responseArr = [];

        //$reponse_url = "https://cafehrvarunagroup.darwinbox.in/JobsApiv2/Joblist";
        $reponse_url = "https://cafehrvarunagroup.darwinbox.in/JobsApiv3/Joblist";
        
        if(!empty($job_id)){
            //$reponse_url = "https://cafehrvarunagroup.darwinbox.in/JobsApiv2/Jobdetail";
            $reponse_url = "https://cafehrvarunagroup.darwinbox.in/JobsApiv3/Jobdetail";
        }
        
        $Uid = config('custom.career_uid');
        $admin_email = config('custom.career_admin_email');
        $secretkey = config('custom.carrer_secretkey');
        $curr_time = time();

        $pass = $admin_email.$secretkey.$curr_time;
        $hash_code = hash("sha512", $pass);

        $dataArr = [];

        $dataArr['timestamp'] = $curr_time;
        $dataArr['hash'] = $hash_code;
        $dataArr['Uid'] = $Uid;
        $dataArr['job_id'] = $job_id;

        $jsoneData = json_encode($dataArr);

        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => $reponse_url,
    ...
    public static function getMapLocations(){

        $result = [];

        $mapLocation = MapLocation::where('status', 1)->get();
    ...
    public static function getCasestudy($id=0, $parent_id=0, $limit='', $params=[]){
    ...
    public static function getMedia($id=0, $parent_id=0, $limit='', $params=[]){
