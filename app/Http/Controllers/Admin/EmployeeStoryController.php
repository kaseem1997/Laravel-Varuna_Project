...
class EmployeeStoryController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $casestudy_query = EmployeeStory::orderBy('created_at','desc');      
        //prd($blogs->toArray());
        ...
        return view('admin.employee_stories.index', $data);
    }
    ...
    public function add(Request $request){
        ...
        $case_study = '';
        $title = 'Add Employee Story';
     
        if(is_numeric($id) && $id > 0){
            $case_study = EmployeeStory::find($id);
            $title = 'Edit Employee Story('.$case_study->title." )";
        }
        if($request->method() == 'POST' || $request->method() == 'post'){
            ...
            //$slug = CustomHelper::GetSlug('employee_stories', 'id', $id, $request->title);

            if(empty($id)){
                $slug = CustomHelper::GetSlug('employee_stories', 'id', $id, $request->title);
            }
            else{
                $slug = CustomHelper::GetSlug('employee_stories', 'id', $id, $request->slug);
            }
            ...
            if(!empty($case_study) && count($case_study) > 0){
                $isSaved = EmployeeStory::where('id', $case_study->id)->update($req_data);
                $msg = "Employee Story has been updated successfully.";
            }
            else{
                $isSaved = EmployeeStory::create($req_data);
                $id = $isSaved->id;
                $msg = "Employee Story has been added successfully.";
            }
            ...
            cache()->forget('case-studies');
                ...
                return redirect(url($this->ADMIN_ROUTE_NAME.'/empoloyee-stories'))->with('alert-success', $msg);
            } else {
                return back()->with('alert-danger', 'The Employee Story could be added, please try again or contact the administrator.');
            }
        }
        ...
        return view('admin.employee_stories.form', $data);
    }
    ...
    public function saveImage($file, $id){
        ...
        if ($file) 
        {
            ...
            $thumb_path = 'employee_stories/thumb/';
        ...
        if($method=="POST"){
            if(is_numeric($id) && $id > 0)
            {
                ...
                if(!empty($case_study) && count($case_study) > 0){
                        ...
                        if(count($case_study) > 0 && !empty($case_study->image))
                        {   
    ...
    if($is_deleted){
        return redirect(url($this->ADMIN_ROUTE_NAME.'/empoloyee-stories'))->with('alert-success', 'The Employee Story has been deleted successfully.');
    }else
    {
        return redirect(url($this->ADMIN_ROUTE_NAME.'/empoloyee-stories'))->with('alert-danger', 'The Employee Story cannot be deleted, please try again or contact the administrator.');
    }
   ...
   public function ajax_change_featured(Request $request){
        ...
        if (is_numeric($id) && $id > 0) {
            ...
            $result['success'] = true;
            $result['msg'] = '<div class="alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Employee Story Updated successfully.</div>';
        }
    ...
    /*ajax_copy_record*/
    public function ajaxCopyRecord(Request $request){
        ...
        if (is_numeric($id) && $id > 0) {
            ...
            $tasks = EmployeeStory::find($id);
            $newTask = $tasks->replicate();
            $newTask->save();

            $result['success'] = true;
            //$result['msg'] = '';
            session()->flash('alert-success', 'Employee Story Copy successfully.');
        }
