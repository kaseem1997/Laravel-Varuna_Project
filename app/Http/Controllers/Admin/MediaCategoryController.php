...
class MediaCategoryController extends Controller{
    ...
    public function index(Request $request){
        ...
        $parent_id = (isset($request->parent_id))?$request->parent_id:'';
            ...
            $category_query = MediaCategory::where(['parent_id'=>$parent_id])->orderBy('id', 'desc');
            ...
            if(is_numeric($parent_id) && $parent_id > 0){
                $parentCategory = MediaCategory::where('id', $parent_id)->first();
            }
        ...
        return view('admin.media_categories.index', $data);
    }
    ...
    public function add(Request $request){
        ...
        if($request->method() == 'POST' || $request->method() == 'post'){
            ...
            if(!empty($category) && count($category) > 0 && $req_id == $id){
                ...
                $isSaved = MediaCategory::where(['id'=>$category->id, 'type'=>$type])->update($req_data);
                ...
                $isSaved = MediaCategory::create($req_data);
            ...
            $page_heading = 'Add Media Category';

            if(isset($category->name)){

                $page_heading = 'Edit Media Category - '.$category->name;
        ...
        $parent_category = '';
        if(is_numeric($parent_id) && $parent_id > 0){
            $parent_category = MediaCategory::find($parent_id);
        }
    ...
    public function delete($id){
        ...
        if ($category->media() && $category->media()->count() > 0) {
            ...
            return back()->with('alert-danger', 'This category cannot be removed because there are currently ' . $category->media()->count() . ' blogs associated with it. Please remove the blogs first.');
        ...
        if ($category->children() && $category->children()->count() > 0) {
            ...
            return back()->with('alert-danger', 'This category cannot be removed because there are currently ' . $category->children()->count() . ' Media sub-categories associated with it. Please remove the Media sub-categories first.');
