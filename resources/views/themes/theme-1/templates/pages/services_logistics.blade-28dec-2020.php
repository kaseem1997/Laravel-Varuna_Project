...
?>
<section class="caseStudies wow fadeInDown"><img alt=" " class="img-fluid" src="/public/assets/themes/theme-1/images/logisticsbanner2.png" />
            ...
            if(!empty($blog_images) && count($blog_images)){
              foreach($blog_images as $bimg){
                if(!empty($bimg->image) && $storage->exists('blogs/'.$bimg->image)){
                  $blog_image = asset('public/storage/blogs/'.$bimg->image);
                  break;
                }
              }
            }
