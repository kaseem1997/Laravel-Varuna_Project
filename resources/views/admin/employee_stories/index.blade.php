@component('admin.layouts.main')
    ...
    @slot('title')
        Admin - Manage Employee Stories - {{ config('app.name') }}
    @endslot
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <h2>Manage Employee Stories
                <a href="{{ route($routeName.'.empoloyee-stories.add').'?back_url='.$back_url }}" class="btn btn-sm btn-success pull-right"><i class="fa fa-plus"></i> Add Employee Stroy</a>
            </h2>
                    ...
                    <div class="table-responsive">
                        ...
                        <form class="form-inline" method="GET">
                            ...
                            <div class="col-md-6">
                                ...
                                <a href="{{url($routeName.'/empoloyee-stories')}}" class="btn resetbtn btn-primary btn1search" >Reset</a>
                            </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <table class="table table-striped">
                        ...
                        foreach($case_studies as $case_study){
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route($routeName.'.empoloyee-stories.edit', $case_study->id.'?back_url='.$back_url) }}" title="Edit Blog"><i class="fas fa-edit"></i></a>

                                    <a href="javascript:void(0)" class="sbmtDelForm"  id="{{$case_study->id}}" title="Delete Blog"><i class="fas fa-trash-alt"></i></i></a>

                                    <a href="javascript:void(0)" class="copy_record"  data-id="{{$case_study->id}}" title="Copy Record"><i class="fas fa-copy"></i></i></a>
                                
                                    <form method="POST" action="{{ route($routeName.'.empoloyee-stories.delete', $case_study->id) }}" accept-charset="UTF-8" role="form" onsubmit="return confirm('Do you really want to remove this Employee Story?');" id="delete-form-{{$case_study->id}}">
                                        {{ csrf_field() }}
                                        {{ method_field('POST') }}
...
<script type="text/javaScript">
    ...
    $(".is_featured").click(function(){
        ...
        conf = confirm("Are you sure to Featured this Employee Story?");
        ...
        if(conf){
            ...
            $.ajax({
                url: "{{ route($routeName.'.empoloyee-stories.ajax_change_featured') }}",
                type: "POST",
    ...
    $(".copy_record").click(function(){
        ...
        conf = confirm("Are you sure to Copy this Employee Story?");
        ...
        if(conf){
            ...
            $.ajax({
                url: "{{ route($routeName.'.empoloyee-stories.ajax_copy_record') }}",
                type: "POST",
                data: {id},
                dataType:"JSON",
                headers:{'X-CSRF-TOKEN': _token},
